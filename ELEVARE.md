# Elevare Project Context

## Mission
Elevare is a language learning and personal development coaching platform with three pillars: Lingua (language fluency), Rise (confidence building), and Flex Nexus (professional communication). Our mission is to help people overcome communication barriers through mental-health-informed coaching that addresses the psychology behind speaking, not just the mechanics.

## Current Goal
**$1,000 MRR in 30 days → $5,000 MRR in 90 days**

## Team
| Person | Role | Responsibilities |
|--------|------|------------------|
| Debby | Founder / Brand | Brand strategy, content, customer acquisition, sales |
| Donald | Technical / Infra | Deployment, Cloudflare, integrations, site maintenance |
| 4 Tutors | Delivery | Coaching sessions, student progress, curriculum delivery |

## Tech Stack
| Phase | Stack | Timeline |
|-------|-------|----------|
| Current | GitHub Pages (darnygirl.github.io/elevare) | Now |
| Month 2 | Astro + Cloudflare Pages + MDash rebuild | Week 5-8 |

## Payment Infrastructure
| Provider | Status | Use |
|----------|--------|-----|
| Whop | Primary | Course sales, memberships |
| Lemon Squeezy | Backup | Alternative payment processor |

## Resources
- **Blockers Sheet:** [PASTE BLOCKERS SHEET URL HERE]
- **Shared Drive:** [PASTE GOOGLE DRIVE LINK HERE]

## Key Pages
- Main Site: https://darnygirl.github.io/elevare/
- AI Tutor: https://darnygirl.github.io/elevare/tutor-agent/tutor-dashboard.html
- Marketing Dashboard: https://darnygirl.github.io/elevare/marketing/marketing-dashboard.html
- Pricing: https://darnygirl.github.io/elevare/frontend/pricing.html

## Notes
- All changes pushed to GitHub auto-deploy to GitHub Pages
- Voice cloning works locally (XTTS v2 on Python 3.11)
- AI Tutor uses OpenAI for responses (requires API key in settings)
- WhatsApp: +601111612082
